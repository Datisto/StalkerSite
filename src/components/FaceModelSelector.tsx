...
export default function FaceModelSelector({
  ...
  const uniqueScrollRef = useRef<HTMLDivElement>(null);
    ...
    <div className="space-y-6">
      <div>
        ...
        <div className="relative group">
          <button
            type="button"
            onClick={() => scroll(regularScrollRef, 'left')}
          ...
          <button
            type="button"
            onClick={() => scroll(regularScrollRef, 'right')}
            ...
            {regularModels.map((model) => {
              ...
              return (
                <button
                  ...
                  className={`relative rounded-lg overflow-hidden border-2 transition-all flex-shrink-0 w-48 ${
                    isSelected
      ...
      {uniqueModels.length > 0 && (
        <div>
          ...
          <div className="relative group">
            <button
              type="button"
              onClick={() => scroll(uniqueScrollRef, 'left')}
              className="absolute left-0 top-1/2 -translate-y-1/2 z-10 bg-black/70 hover:bg-black/90 p-2 rounded-full opacity-0 group-hover:opacity-100 transition-opacity"
            >
              <ChevronLeft className="w-6 h-6" />
            </button>
            <button
              type="button"
              onClick={() => scroll(uniqueScrollRef, 'right')}
              className="absolute right-0 top-1/2 -translate-y-1/2 z-10 bg-black/70 hover:bg-black/90 p-2 rounded-full opacity-0 group-hover:opacity-100 transition-opacity"
            >
              <ChevronRight className="w-6 h-6" />
            </button>
